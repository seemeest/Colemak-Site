...
Route::controller(UsersController::class)->middleware('SetResponseHeaders')->prefix('users')->group(function () {
	Route::get('/', 'show')->name('users.show');
});

Route::controller(AuthorizationsController::class)->middleware('SetResponseHeaders')->prefix('users')->group(function () {
	Route::post('/registration', "registration")->name('users.registration');
	Route::post('/login', 'login')->name('users.login');
	Route::post('/logout', 'logout')->name('users.logout');
});

Route::controller(GroupsController::class)->middleware('SetResponseHeaders')->prefix('groups')->group(function () {
...
Route::controller(EventsController::class)->middleware('SetResponseHeaders')->prefix('events')->group(function () {
